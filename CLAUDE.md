...
- **Phase 3 deployment artefacts (B-78)** — `deploy/bicep/modules/azure-openai.bicep` provisions a `Microsoft.CognitiveServices/accounts` (kind `OpenAI`, S0) plus two GPT-4o-mini deployments named `extractor` and `disambiguator`. The two deployments must be created sequentially — Cognitive Services rejects concurrent deployment creates on the same account, so `disambiguatorDeployment` carries an explicit `dependsOn` to `extractorDeployment`. Default capacities: 50K TPM extractor / 30K TPM disambiguator (Bicep params `extractorCapacity` / `disambiguatorCapacity`). Module is opt-in via `enableAzureOpenAI` parameter on `main.bicep` (false in `main.bicepparam`, true in `main-prod.bicepparam`); test environments skip the module so we don't burn AOAI quota for empty environments. `publicNetworkAccess` defaults to `Enabled` until VNet integration lands (B-92 follow-up); production should flip to `Disabled` in tandem with private-endpoint setup.
- **Phase 3 Key Vault secrets** — When AOAI is enabled, `main.bicep` writes two Key Vault secrets via the canonical `--` → `:` translation: `Providers--AzureOpenAI--Endpoint` (non-sensitive but persisted as a secret for consistent config-source handling) and `Providers--AzureOpenAI--ApiKey` (truly secret; consumed by `AiExtractorClient` / `LlmDisambiguatorClient` until managed-identity migration). Deployment names (`extractorDeploymentName`, `disambiguatorDeploymentName`) are output as plain Bicep outputs because they're not secrets. **Conditional `module` outputs require `!` non-null assertion** — `azureOpenAI!.outputs.endpoint` is the correct syntax in Bicep when the module is gated on a boolean parameter.
- **Phase 3 smoke test (`deploy/scripts/smoke-test-phase3.sh`)** — Curl + jq script that exercises the full Phase 3 surface: `/health`, Deep trace submission + completion polling (60 s budget), DE profile lookup (Tier 2 scraping verification), `/api/changes/stats` + `/api/validation/stats` aggregates, since-filtered Change Feed, and a manual override audit round-trip including verification that `GET /api/profiles/{id}/history` shows the entry with `changeType = ManualOverride` and `detectedBy` starting with `manual-override:apikey:`. Designed as the deploy-gate after `az deployment group create`; exits non-zero on first failure with a clear FAIL message. Submitted payloads use the same fictitious values as the B-86 k6 scripts (`Contoso International Testing Ltd.`) — never put real company data in smoke tests because the waterfall writes to CKB and publishes to Service Bus.
- **Phase 3 performance tuning guide (`docs/performance/phase-3-tuning.md`)** — Captures the SLO targets, App Service plan recommendations (P1V3 prod, Always-Ready ≥ 1, **stay at Workers = 1 until B-92 ships the distributed re-validation lock**), Server GC defaults, cache-warming requirements, per-provider Polly resilience defaults (AOAI extractor 20 s timeout, disambiguator 5 s, Handelsregister 60 req/h legal limiter as `PermitsPerSecond = 0.0166`), Service Bus consumer concurrency uplifts (Phase 3 `MaxConcurrentCalls = 4`, `LockDuration = 60 s`), and the alert thresholds that go into `monitoring.bicep` in B-92. **Do not enable App Service auto-scale before B-92 distributed lock** — multi-instance scaling without it duplicates re-validation runs.
- **Manual field override audit (B-85)** — `PUT /api/profiles/{id}/fields/{field}` accepts a `{value, reason}` body and routes through `OverrideFieldCommand` → `OverrideFieldHandler`. Caller identity is read **only** from `HttpContext.Items[ApiKeyAuthMiddleware.CallerFingerprintItemKey]` — never from the body. Handler builds a `TracedField<string>` (Confidence = 1.0, Source = `"manual-override:apikey:XXXX"`), calls `CompanyProfile.UpdateField` (existing change-detection path), then `ChangeEvent.MarkAsManualOverride()` reclassifies the resulting event so `GET /api/profiles/{id}/history` and downstream Service Bus consumers see `ChangeType.ManualOverride`. **Severity is unchanged** — overriding `EntityStatus` still publishes through the Critical handler. **Whitelist** in `OverrideFieldHandler.OverridableFields` covers string-typed scalar fields (`LegalName`, `TradeName`, `TaxId`, `LegalForm`, `Phone`, `Email`, `Website`, `Industry`, `EmployeeRange`, `EntityStatus`, `ParentCompany`); Address / Location are excluded (typed bodies, separate block), Officers is excluded (GDPR-gated), `RegistrationId` is excluded (not a `TracedField<T>`). `Reason` is logged-only — persisting it to `ChangeEvent` requires a schema change tracked as a B-85 follow-up.
- **`ChangeType.ManualOverride` (B-85)** — Added as enum value 3 in both `Tracer.Domain.Enums.ChangeType` and `Tracer.Contracts.Enums.ChangeType` (mirroring rule). `ChangeEvent.MarkAsManualOverride()` is the only way to set it — idempotent, leaves Severity intact. All filters / DTOs / SignalR handlers tolerate it via the existing `JsonStringEnumConverter` registration; do not add new branches except where audit semantics actually differ from `Updated`.
- **Lightweight re-validation runner (B-66)** — `LightweightRevalidationRunner` (Application, Scoped, internal) refreshes only `TracedField<T>.EnrichedAt` on expired fields without calling any provider; semantic is "value still correct as of now". Used by `CompositeRevalidationRunner` when `expiredFields.Count <= Revalidation:Lightweight:Threshold` (default 2). **Save boundary:** lightweight does NOT call `IUnitOfWork.SaveChangesAsync` — the scheduler flushes on profile exit; this matches the `IRevalidationRunner` contract for in-memory runners. `ValidationRecord` (`ValidationType.Lightweight`, `ProviderId = "revalidation-lightweight"`) is added to the scoped DbContext but waits for the scheduler save. `FieldsChanged = 0` by definition (lightweight doesn't change values). Officers / RegistrationId are not refreshed (GDPR-gated and non-`TracedField<T>` respectively).
- **Composite revalidation runner (B-66)** — `CompositeRevalidationRunner` is the production `IRevalidationRunner` registered in `ApplicationServiceRegistration` (Scoped). Dispatches by `IFieldTtlPolicy.GetExpiredFields(profile, now).Count`: ≤ `Revalidation:Lightweight:Threshold` → lightweight, else deep. When `Revalidation:Lightweight:Enabled = false` the composite always delegates to deep (pre-B-66 behaviour). Scheduler binds only to `IRevalidationRunner` so any future runner mode (e.g. single-provider targeted) drops into the composite without changing the scheduler. Logging is debug-level — composite decisions are high-frequency and would swamp info logs at 100+ profiles/tick.
- **`CompanyProfile.RefreshFieldEnrichedAt(FieldName, DateTimeOffset)`** — Domain method added in B-66 that mutates only the `EnrichedAt` timestamp of a single field, monotonically (silently no-ops if `now` is older than current). **Never raises a domain event** — value is unchanged. Idempotent; safe to call on a null field. The switch covers all 14 `TracedField<T>` properties; `FieldName.Officers` and `RegistrationId` fall through to default (`break`) since they are not refresh targets — Officers is GDPR-gated and `RegistrationId` is a plain string column without `EnrichedAt`.
